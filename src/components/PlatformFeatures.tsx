...
const PlatformFeatures: React.FC<PlatformFeaturesProps> = ({ data }) => {
  return (
    <section className="py-24 px-4 md:px-8 bg-[#0e0e11] text-white">
      <div className="container mx-auto px-6">

        {/* Title */}
        <h2 className="text-4xl md:text-5xl font-bold text-center text-gradient-primary mb-16">
          {data.title}
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-6xl mx-auto">
          {data.items.map((item, index) => (
            ...
            >
              ...
              {/* Title */}
              <h3 className="text-2xl font-semibold text-gradient-primary mb-3">
                {item.title}
              </h3>
